...
(function() {
    ...
    var _registerHoverSkinEvent = function $KE_registerHoverSkinEvent(body) {
        var $K = kony.$kwebfw$, $KD = $K.dom, touchFired = false;

        $KD.on(body, 'touchstart mousemove', 'hskin', function(evt) {
            var timeout = null;

            if(evt.type === 'touchstart') touchFired = true;

            //On touch devices "mousemove" fires only once.
            //And that too after "touchstart" is fired.
            if(evt.type === 'mousemove') {
                if(touchFired === true) {
                    ...
                    timeout = setTimeout(function() {
                        ...
                        _handleSkinEvent.dealWithHoverSkin(evt);
                    }, 17);
                }
            }
        }, {passive:false});
