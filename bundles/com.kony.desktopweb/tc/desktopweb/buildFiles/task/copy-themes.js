...
async function copyThemesLibModeCSS(srcDir, destDir, themes, shouldMinify) {
    ...
    promise = new Promise((resolve, reject) => {
        fs.closeSync(
            fs.openSync(
                destDir+'/konydesktop.css',
                'w'
            )
        );
        console.log("Succesfully created konydesktop.css");
        resolve();
    });
...
async function copyThemesLibModeJS(srcDir, destDir, themes, shouldMinify) {
    ...
    themes.forEach(async theme => {
        ...
        if(theme === 'default') {
            asset = srcDir+'/konydesktop.css';

            promise = new Promise((resolve, reject) => {
                fs.copy(asset, (destDir+'/konydesktop.css'), async err => {
                    if(err) {reject(); throw err;}
