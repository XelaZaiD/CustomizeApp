...
const populateWebapp = async (prop) => {
    return new Promise(async (resolve, reject) => {
        ...
        let webWorkerDir = loc.ipJssrcPath(prop);
        if(fs.existsSync(webWorkerDir+'/workerthreads')) {
            let stats = fs.statSync(webWorkerDir+'/workerthreads'); 
            if(stats.isDirectory()){
                await populatedWebWorkers(
                    prop,
                    webWorkerDir,
            ...
            console.log("No Worker Threads present");
        }
